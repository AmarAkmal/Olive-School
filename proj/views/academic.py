...
@bp_academic.route('/add_invoice', methods=['POST'])
def add_invoice():
...
# Inter mobile
@bp_academic.route('/get_invoice', methods=['GET'])
def get_invoice():
...
@bp_academic.route('/update_invoice', methods=['POST'])
def update_invoice():
    response = dict()
    data = json.loads(request.form['data'])
...
@bp_academic.route('/deleted_account/<data>', methods=['DELETE'])
